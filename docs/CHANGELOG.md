...
- 恢复到稳定的 `0.1.26` 打包基线，避免影响状态栏与扩展激活。
- Dashboard 新增独立的 auth 导出入口，可直接生成仅包含 `auth.json` 内容的 JSON 数组 `[{...}, {...}]`。
- 保留原有的 OmniRoute 批量导出 JSON，不改变既有分享/导入流程。

### 说明

- 本次新增的是“纯 auth 列表”导出，适合复制粘贴到只接受 `auth.json` 数组的批量导入场景。
